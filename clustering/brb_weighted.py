...
os.environ["CUDA_VISIBLE_DEVICES"] = "1,2,3,4,5,6,7"  # gatedate based on available GPUs
...
def process_chunk_on_gpu(chunk_file, centroids, k, device, gpu_id, iteration, percentile, max_cluster_size, batch, num_chunks, sparsity_threshold = 0.4):
    ...
    with torch.cuda.device(device):
        ...
        for i in range(k):
            ...
            diff = (binary_G == 1).to(binary_G.dtype) * (binary_G - binary_local_centroids[i])  # Compute differences for 1s only
            ...
            if cluster_points.size(0) > 0:
                ...
                if cluster_points.size(0) > max_cluster_size:
                    ...
                    if (torch.sum(gatedate[i] == 0) / gatedate[i].numel() + 0.1 < sparsity_threshold) :  # Threshold for imbalance
                        ...
                        #     temp_new_centroid = (new_centroid > threshold).float()
                        #     num_points_in_centroid += 1
                        #     #new_centroid = (new_centroid >= 0.5).float()
                            
                        #     # Check sparsity of the new centroid
                        #     sparsity = torch.sum(temp_new_centroid == 0).item() / new_centroid.numel()
...
def brb_kmeans_with_percentile(chunk_files, k, num_chunks, max_iter=100, seed=42, percentile=0.5):
    ...
    centroids = torch.load("clustering_results_50_mistral_weighted/initialized_centroids_gate_2048_kmeanspp.pt", map_location=device_list[0])
    print(f"Loaded pre-initialized centroids from 'clustering_results_50_mistral_weighted/initialized_centroids_gate_2048_kmeanspp.pt'.")
    ...
    for iteration in range(max_iter):
        ...
        with ThreadPoolExecutor(max_workers=num_gpus) as executor:
            ...
            for batch_idx, batch in enumerate(chunk_batches):
                ...
                centroids = synchronize_centroids(centroid_gatedates, device_list[0], percentile)
                k = centroids.size(0)
                print(f"centroids length after {iteration + 1} iteration, {centroids.shape}.")
                torch.save(centroids, "clustering_results_50_mistral_weighted/centroids_gate_2048_sparsity_40_new.pt")
...
if __name__ == "__main__":
    ...
    centroids_gate = brb_kmeans_with_percentile(chunk_files, k=2048, max_iter=200, percentile=0.4, num_chunks=num_chunks)
    torch.save(centroids_gate, "clustering_results_50_mistral_weighted/centroids_gate_2048_sparsity_40_new.pt")
